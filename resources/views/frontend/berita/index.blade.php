...
@section('content')
    ...
    <section class="breadcrumbs"
        style="background: linear-gradient(135deg, #DC143C 0%, #B71C1C 100%); padding: 80px 0 60px; margin-top: 0;">
        <div class="container">
            <div class="row">
                <div class="col-lg-12">
                    <div class="breadcrumbs-content text-center">
                        <h1 class="page-title"
                            style="color: #fff; font-size: 3rem; font-weight: 700; margin-bottom: 10px; text-transform: uppercase; letter-spacing: 2px;">
                            Berita</h1>
                        <p style="color: rgba(255, 255, 255, 0.9); font-size: 1.1rem; margin: 0;">Informasi terkini dari
                            Palang Merah Indonesia</p>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="col-lg-4 col-md-12 col-12" style="margin-bottom: 30px;">
                    <div class="blog-sidebar" style="position: sticky; top: 20px;">
                        ...
                        <div class="single-widget categories">
                            ...
                            <ul>
                                <li>
                                    <a style="background-color: #fff; !important" href="{{ route('frontend.berita') }}"
                                ...
                                <div class="col-lg-6 col-md-6 col-12 mb-4">
                                    <div class="single-blog" style="margin-bottom: 30px;">
                                        <div class="blog-image-wrapper"
                                            style="position: relative; overflow: hidden; border-radius: 8px 8px 0 0; height: 220px; background: #f0f0f0;">
                                            <a href="{{ route('frontend.berita.detail', $berita->slug ?? '#') }}"
                                            ...
                                            ');
                                                ...
                                                @endphp
                                                <img src="{{ $gambarUrl }}" alt="{{ $berita->judul ?? 'Berita' }}"
                                            ...
                                            @if (isset($berita->kategori_berita_id) && $berita->kategori)
                                                <div class="blog-tag"
                                                    style="position: absolute; top: 12px; left: 12px; background: rgba(220, 20, 60, 0.95); color: #fff; padding: 6px 14px; border-radius: 20px; font-size: 0.7rem; font-weight: 700; text-transform: uppercase; z-index: 10; box-shadow: 0 2px 8px rgba(0,0,0,0.2);">
                                        ...
                                        <div class="blog-content">
                                            <div class="blog-meta">
                                                <span class="date"><i class="lni lni-calendar"></i>
                                                    {{ $berita->published_at ? $berita->published_at->format('d M Y') : ($berita->created_at ? $berita->created_at->format('d M Y') : '') }}</span>
                                                <span class="author"><i class="lni lni-user"></i>
                                                    {{ $berita->penulis ?? 'Admin' }}</span>
                                            </div>
                                            <h4 class="blog-title">
                                                <a
                                                    href="{{ route('frontend.berita.detail', $berita->slug ?? '#') }}">{{ $berita->judul ?? 'Judul Berita' }}</a>
                                            </h4>
                                            <p class="text">
                                                {{ Str::limit($berita->ringkasan ?? (isset($berita->konten) ? strip_tags($berita->konten) : ''), 100) }}
                                            </p>
                                            <a href="{{ route('frontend.berita.detail', $berita->slug ?? '#') }}"
                                                class="more">Baca Selengkapnya <i class="lni lni-arrow-right"></i></a>
                                        </div>
                                    </div>
                                </div>
                            @endforeach
