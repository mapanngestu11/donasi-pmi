                ...
                <div class="card-body">
                    <form action="{{ route('admin.user.update', $user->id) }}" method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <small class="form-text text-muted">Kosongkan jika tidak ingin mengubah password. Minimal 6
                                karakter jika diisi.</small>
                            @error('password')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control @error('hak_aksis') is-invalid @enderror" id="hak_aksis"
                                ...
                                <option value="superadmin"
                                    {{ old('hak_aksis', $user->hak_aksis) == 'superadmin' ? 'selected' : '' }}>Kepala PMI
                                </option>
                                <option value="admin"
                                    {{ old('hak_aksis', $user->hak_aksis) == 'admin' ? 'selected' : '' }}>Admin</option>
                                <option value="user"
                                    {{ old('hak_aksis', $user->hak_aksis) == 'user' ? 'selected' : '' }}>User</option>
                            </select>
                            @error('hak_aksis')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        @if ($user->last_login)
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control"
                                    value="{{ $user->last_login->format('d/m/Y H:i:s') }}" readonly>
                            </div>
                        @endif
